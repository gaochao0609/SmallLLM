...
class MOEFeedForward(nn.Module):
        ...
        if self.training:
            ...
            for i, expert in enumerate(self.experts):
                ...
                if torch.any(expert_mask):
                    y[expert_mask] = expert(x[expert_mask])
    ...
    def moe_infer(self, x, flat_expert_indices, flat_expert_weights):
        ...
        for i, end_idx_tensor in enumerate(cumulative):
            end_idx = int(end_idx_tensor.item())
            start_idx = int(cumulative[i - 1].item()) if i > 0 else 0
            ...
            scatter_index = exp_token_idx.view(-1, 1).expand(-1, x.shape[-1])
            expert_cache.scatter_add_(0, scatter_index, expert_out)
    ...
    def forward(self,
        ...
        if past_key_values is not None:
            if Cache is not None and isinstance(past_key_values, Cache):
                ...
                past_key_values = list(past_key_values.layers)
        if past_key_values is None:
            past_key_values = [None] * len(self.layers)
        else:
            past_key_values = list(past_key_values)
            if len(past_key_values) < len(self.layers):
                past_key_values.extend([None] * (len(self.layers) - len(past_key_values)))
